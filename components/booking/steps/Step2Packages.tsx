...
export default function Step2Packages({ data, onNext, onBack }: Step2Props) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-6">
      ...
      <p className="text-xs text-gray-400 mb-6">
        List every item you want on this trip. Each package gets
        its own entry and tracking number.
      </p>
      ...
      <div className="flex flex-col gap-2 mb-4">
        {packages.map((pkg) => (
          ...
          >
            <div className="flex-1 min-w-0">
              <p className="text-sm text-gray-900 truncate">{pkg.description}</p>
              <p className="text-xs text-gray-400">
                {pkg.boxes} box{pkg.boxes > 1 ? "es" : ""} · {pkg.weight} kg
              </p>
            </div>
            <span className="text-xs font-medium text-green-700 bg-green-50 px-2 py-1 rounded-lg whitespace-nowrap">
              {pkg.weight} kg
            </span>
            <button
        ...
        {packages.length === 0 && (
          <div className="text-center py-8 border border-dashed border-gray-200 rounded-xl">
            ...
            <p className="text-xs text-gray-300 mt-1">Add your first package below</p>
          </div>
        )}
      </div>
      ...
      {/* Total weight */}
      {packages.length > 0 && (
        <div className="flex items-center justify-between bg-green-50 border border-green-100 rounded-xl px-4 py-3 mb-6">
          <div>
            <p className="text-xs text-green-700 font-medium">Total cargo weight</p>
