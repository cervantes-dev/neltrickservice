...
export default function Step4Addresses({ data, onNext, onBack }: Step4Props) {

    // pre-fill sender from customer selected in Step 1
    const [senderName,       setSenderName]       = useState(
        data.senderName || data.customer?.name    || ""
    )
    const [senderContact,    setSenderContact]    = useState(
        data.senderContact || data.customer?.phone || ""
    )
    const [senderAddress,    setSenderAddress]    = useState(
        data.senderAddress || data.customer?.address || ""
    )
    const [senderCity,       setSenderCity]       = useState(
        data.senderCity || data.customer?.city    || ""
    )
    ...
    return (
        <div className="bg-white rounded-2xl border border-gray-100 p-6">
            ...
            <p className="text-xs text-gray-400 mb-6">
                Sender details are pre-filled from the selected customer.
                Update if needed.
            </p>
